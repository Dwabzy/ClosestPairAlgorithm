<script>
    ...
    */
    async function animateFrame(frame, duration) {
      ...
      else if (frame.type === "bruteforce")
        await bruteForceVisualization(frame, duration);
      else if (frame.type === "compare")
        await comparisonVisualization(frame, duration);
      else if (frame.type === "strip") await stripVisualization(frame, duration);
      else if (frame.type === "stripClosest")
        await stripClosestVisualization(frame, duration);
    }
    ...
    async function changeFrame(event) {
      if (event.detail === "oneFrameForward") currentFrame += 1;
        ...
        // When moving backwards ( I spent hours on this nonsense cuz I suck at organizing things, Yay!)
          ...
          let currentFrameObject = history.find(
            (item) => item.frameNumber === currentFrame
          );
          ...
          // box floating near the line.
            ...
            canvas.remove(currentFrameObject.line);
            distanceBoxes = distanceBoxes.filter(
              (box) => box !== currentFrameObject.box
            );
           ...
           */
            ...
            */
            ...
            if (currentFrameObject.hasCloserPointsInStrip) {
              ...
              distanceBoxes = [
                ...distanceBoxes,
                currentFrameObject.longerLine.box,
              ];
              distanceBoxes = distanceBoxes.filter(
                (box) => box !== currentFrameObject.box
              );
    ...
    */
    async function divideVisualization(
    ...
    */
    async function bruteForceVisualization(
      { frameNumber, p1, p2, elements, distance },
      duration
    ) {
      let { x: x1, y: y1 } = p1;
      let { x: x2, y: y2 } = p2;
    ...
    */
    async function stripVisualization(
      { frameNumber, x1, y1, x2, y2, elements },
      duration
    ) {
    ...
    */
    async function stripClosestVisualization(
      ...
      if (hasCloserPointsInStrip) {
        ...
        distanceBoxes = distanceBoxes.filter(
          (box) => box.distanceLine !== lineObject.line
        );
    ...
    */
    async function restartAnimation() {
      ...
      for (let i = 0; i < currentMidLines.length; i++)
        canvas.remove(currentMidLines[i].line);
      canvas.remove(stripRegion);
    ...
    <div class="canvas-box">
      ...
      {#each distanceBoxes as box}
        <div
          ...
          style="top:{box.top}px; left:{box.left -
            40}px;transform: rotate({box.angle % 90}deg"
        >
          <div class="distance">
